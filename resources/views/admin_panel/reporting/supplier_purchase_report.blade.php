    ...
    <div class="body-wrapper">
        <div class="bodywrapper__inner">
            <div class="d-flex mb-30 flex-wrap gap-3 justify-content-between align-items-center">
                <h6 class="page-title">Supplier Purchase Report</h6>
            </div>
...
<script>
    $(document).ready(function() {
        $('#filterpurchaseBtn').click(function() {
            ...
            let supplier = $('#supplier').val(); // ✅ Supplier value pick

            $.ajax({
                url: "{{ route('filter.purchase.suplir') }}",
                method: "GET",
                data: {
                    start_date: start_date,
                    end_date: end_date,
                    supplier: supplier // ✅ Send supplier in request
                },
                beforeSend: function() {
                ...
                success: function(response) {
                    ...
                    if (response.length > 0) {
                        response.forEach(purchase => {
                            const itemCategory = purchase.item_category ? JSON.parse(purchase.item_category).join(", ") : 'N/A';
                            const itemName = purchase.item_name ? JSON.parse(purchase.item_name).join(", ") : 'N/A';
                            const quantities = purchase.quantity ? JSON.parse(purchase.quantity) : ['0'];
                            const prices = purchase.price ? JSON.parse(purchase.price) : ['0'];
                            const totals = purchase.total ? JSON.parse(purchase.total) : ['0'];

                            const quantitySum = quantities.reduce((sum, val) => sum + parseFloat(val || 0), 0);
                            const totalSum = totals.reduce((sum, val) => sum + parseFloat(val || 0), 0);
                        ...
                        tableData = '<tr><td colspan="9" class="text-center">No purchases found for the selected filters.</td></tr>';
                    }
                    ...
                    $('.card').last().find('.card-body').append(`
                <div id="reportTotals" class="mt-3">
                    <h5 class="text-success">Total Quantity: <strong id="totalQuantity">${totalQuantity}</strong></h5>
                    <h5 class="text-success">Total Purchase Amount: <strong id="totalAmount">${totalAmount.toFixed(2)}</strong></h5>
                </div>
            `);
                },
                error: function(xhr) {
